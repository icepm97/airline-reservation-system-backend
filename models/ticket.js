...
const getTicketDetail = async (ticket_id) => {
        let { rows } = await connection.query('SELECT "class".name AS "class", ticket.passenger_id, ticket.seat_id, ticket.booking_id , customer.first_name AS bookedBy, passenger.first_name, passenger.last_name, passenger.passport_no, age(booking.date, passenger.birthday) as age, passenger.country AS passenger_country, seat.aircraft_model, flight.journey_duration, flight.departure_time, flight_id, schedule.state, aircraft_model.model, aircraft_model.brand, booking.date, price.amount AS price, route_id, A.short_code AS origin_shortCode, A.country AS origin_country, A.name AS origin_name, C.name AS origin_location, B.short_code AS destination_shortCode, B.country AS destination_country, B.name AS destination_name, D.name AS destination_location  FROM ticket NATURAL JOIN passenger NATURAL JOIN seat NATURAL JOIN "class" NATURAL JOIN flight NATURAL JOIN route NATURAL JOIN price NATURAL JOIN schedule NATURAL JOIN booking JOIN customer ON customer.customer_id = booking.customer_id, airport A, airport B, location C, location D, aircraft_model WHERE ticket.ticket_id = $1 and A.short_code=route.origin_id AND B.short_code = route.destination_id AND C.location_id=A.location_id AND D.location_id=B.location_id and aircraft_model.model_id=flight.aircraft_model', [ticket_id])
        if(rows.length==1){
